...
第一个参数写的是 `1L`，不是 `1`。末尾的 `L` 表示这个数字是 `long` 类型。

Java 里直接写出来的数字有默认类型，加后缀可以改变它：
...
小写的 `l` 也合法，但 `1l` 容易和数字 `11` 看混，所以统一写大写 `L`。

### 为什么不能写成 1

构造方法的第一个参数类型是 `Long`。传 `1` 进去，编译器需要连着做两件事：

```text
int 1  →（变宽）→ long  →（装箱）→ Long
```

Java 不允许这两步连着做。自动装箱只认类型完全对应的那一对：
...
```text
不兼容的类型: int无法转换为java.lang.Long
```

写成 `1L` 之后，它本身就是 `long`，装箱成 `Long` 属于对应的那一对，编译通过。

判断规律：某个位置要求 `Long` 或 `long` 时，数字就要带 `L`。后面所有出现 `new Book(1L, ...)`、`findById(1L)` 的地方都是同一个原因。
...
`Book` 的 `id` 用 `Long` 而不是 `long`，是为了能表示“还没有编号”。后面接入数据库时，新增图书会先把 id 设成 `null`，再由数据库生成真正的编号。如果字段类型是 `long`，它最小只能是 0，无法表达“暂时没有值”。

`title` 和 `author` 用的 `String` 也是对象类型，同样可以为 `null`。
